	...
	<div class="row row-content">
		<div class="col-xs-12 col-sm-6 col-sm-offset-3">
			<?php 
				...
				$_SESSION["usu_act_correo"] = $_POST["usu_nuevo_correo"];
				$_SESSION["usu_act_password"] = $_POST["usu_password_nuevo"];
				## Conexion con la BD
				$cn = connection();
				## Query de validacion para el correo electronico, una vez te registres por primera vez
				$validation = "SELECT usu_correo, usu_password, usu_privilegio FROM usuario WHERE usu_correo = ? AND usu_password = ?;";
				$stmt = $cn->prepare($validation);
				$stmt->bind_param("ss", $_SESSION["usu_act_correo"], $_SESSION["usu_act_password"]);
				$stmt->execute();
				$stmt->bind_result($real_mail, $pass, $priv);
				$stmt->fetch(); 
				$cn->close();
				$stmt->close();
				if ($real_mail == $_SESSION["usu_act_correo"] && $pass == $_SESSION["usu_act_password"] && $priv == 1){
					header("Location: admin_sesion.php");
				}
				else if($real_mail == $_SESSION["usu_act_correo"]  && $pass == $_SESSION["usu_act_password"]){
					header("Location: sesion_solo.php");
					}
				else{
					...
					o la contraseñaa son incorrectos, inténtelo nuevamente</div>";
