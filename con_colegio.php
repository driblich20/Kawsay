...
<div class="container">
	<div class="row row-content">
		<div class="col-xs-4">
			<h2 style="color:floralwhite;text-align:center">Actualiza los datos de los estudiantes</h2>
		</div>
		<div class="col-xs-8">
			<form class="form-vertical" role="form">
							<div class="form-group">
								<div class="col-xs-12 col-sm-8">
									<input type="text" class="form-control" name="busqueda" onkeyup="busca_estudiante(this.value)" id="busqueda_input_est" placeholder="Busque a un usuario por su correo" required>
									<span id="nueva_busq"></span>
								</div>
							</div>
							<a type="button" class="btn btn-default" id="boton_nuevo_pass" name="submit" onclick="mostrar_busqueda_est()" value="Registrate">Buscar <span class='glyphicon glyphicon-search'></span></a>
			</form>
		</div>
	</div>
	...
	<div class="row row-content">
		...
		<div class="col-xs-12 col-sm-10">
			...
			<table class="table table-hover">
				<?php 
					...
					$cn = connection();
					$selection = "SELECT col_usu_correo, col_usu_nombre, col_id, col_fecha_log FROM colegio";
					$result = $cn->query($selection);
							if($result->num_rows >0 ){
								echo "<thead><tr class='info'><th>Colegio</th><th>Correo electrónico de estudiante</th><th>Nombre de usuario</th><th>Fecha de registro</th><th>Cambiar</th><th>Eliminar</th></tr></thead>";
								while($row  = $result->fetch_assoc()){
									$con = connection();
									$col_nom = "SELECT col_nombre FROM colegios WHERE col_id = ?;";
									$stmt = $con->prepare($col_nom);
									$stmt->bind_param("i", $row["col_id"]);
									$stmt->execute();
									$stmt->bind_result($col);
									$stmt->fetch();
									echo "<tbody><tr><td>".$col."</td><td>".$row["col_usu_correo"]."</td>".
										 "<td>".$row["col_usu_correo"]."</td>".
										 "<td>".$row["col_fecha_log"]."</td>".
										 "<td><a type='button' class='btn btn-info btn-xs' href='eliminar_est.php?mail=".$row["col_usu_correo"]."&nom=".$row["col_usu_nombre"]."'><span class='glyphicon glyphicon-refresh'></span></a></td>".
										 "<td><a type='button' class='btn btn-danger btn-xs' href='eliminar_est.php?mail=".$row["col_usu_correo"]."'><span class='glyphicon glyphicon-remove'></span></a></td></tr></tbody>";
									$con->close();
									$stmt->close();
		...
		<div class="col-xs-12 col-sm-2">
				<?php
					$con = connection();
					$q = "SELECT COUNT(col_usu_correo) AS 'NumeroUsuarios' FROM colegio;";
					$result = $con->query($q);
